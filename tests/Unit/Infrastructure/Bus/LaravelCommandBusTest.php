...
class LaravelCommandBusTest extends MockeryTestCase
{
	...
	public function testDispatchResolvesAndExecutesHandler(): void
	{
		...
		$handler = new class {
			...
			{
				return CommandResult::success(null, $command->data);
			}
		};
		...
		$this->container->shouldReceive('make')->once()->with($handlerClass)->andReturn($handler);
	...
	public function testDispatchWithCustomHandler(): void
	{
		...
		// Configurar el mock del container
		$this->container->shouldReceive('make')->once()->withAnyArgs()->andReturn($handler);
	...
	public function testDispatchWithMultipleCommands(): void
	{
		...
		for ($i = 0; $i < 3; $i++) {
			...
			$handlers[] = new class {
				public function __invoke($command)
				{
					return CommandResult::success((string) $command->id);
				}
			};
		}
