...
class CreateContractRequestTest extends TestCase
{
    ...
    public function testGetters(): void
    {
        ...
        $this->assertEquals($pacienteId, $request->getPacienteId());
        $this->assertEquals($servicioId, $request->getServicioId());
        $this->assertEquals($fechaInicio, $request->getFechaInicio());
        $this->assertEquals($fechaFin, $request->getFechaFin());
    }
    ...
    public function testPrepareForValidation(): void
    {
        ...
        $this->assertEquals($pacienteId, $request->getPacienteId());
        $this->assertEquals($servicioId, $request->getServicioId());
        $this->assertEquals(new \DateTimeImmutable($fechaInicio), $request->getFechaInicio());
        $this->assertEquals(new \DateTimeImmutable($fechaFin), $request->getFechaFin());
    }
    ...
    public function testPrepareForValidationWithoutFechaFin(): void
    {
        ...
        $this->assertEquals($servicioId, $request->getServicioId());
        $this->assertEquals(new \DateTimeImmutable($fechaInicio), $request->getFechaInicio());
        $this->assertNull($request->getFechaFin());
    }
}
