...
class CreateServiceHandlerTest extends MockeryTestCase
{
    ...
    public function testHandleCreatesServiceWhenCatalogIsActive(): void
    {
        ...
        $catalog->shouldReceive('isActive')
               ->once()
               ->andReturn(true);
    ...
    public function testCreatedServiceHasCorrectData(): void
    {
        ...
        $this->serviceRepository->shouldReceive('save')
            ->once()
            ->with(Mockery::on(function (Service $service) use (&$savedService) {
        ...
        $this->assertEquals(100.00, $savedService->getCosto()->getMonto());
        $this->assertEquals('BOB', $savedService->getCosto()->getMoneda());
        $this->assertEquals('tipo-123', $savedService->getTipoServicioId());
