...
class GetServiceDetailsHandlerTest extends MockeryTestCase
{
    ...
    public function testHandleReturnsServiceDTOWhenServiceExists(): void
    {
        ...
        $service->shouldReceive('getCosto')->andReturn(
            new ServiceCost(100.00, 'BOB', $this->vigencia)
        );
        $service->shouldReceive('getTipoServicioId')->andReturn('tipo-123');
        ...
        $this->assertEquals(100.00, $result->monto);
        $this->assertEquals('BOB', $result->moneda);
        $this->assertEquals($this->vigencia, $result->vigencia);
        $this->assertEquals('tipo-123', $result->tipo_servicio_id);
