...
class ListActiveServicesHandlerTest extends MockeryTestCase
{
	...
	protected function setUp(): void
	{
		...
		$this->service2
			->shouldReceive('getCosto')
			->andReturn(new ServiceCost(200.0, 'BOB', $this->vigencia));
		$this->service2
			->shouldReceive('getTipoServicio')
			->andReturn(TipoServicio::fromString('catering'));
	...
	public function testHandleReturnsAllActiveServicesWhenNoCatalogId(): void
	{
		...
		$this->assertEquals(100.0, $result[0]->monto);
		$this->assertEquals('BOB', $result[0]->moneda);
		$this->assertEquals($this->vigencia, $result[0]->vigencia);
		$this->assertEquals('asesoramiento', $result[0]->tipo_servicio_id);
